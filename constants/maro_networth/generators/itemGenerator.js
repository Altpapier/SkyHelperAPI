    ...
    for (let item of items) {
        if (item.tag?.ExtraAttributes?.id === 'PET' && item.tag?.ExtraAttributes?.petInfo) {
            ...
            if (ExtraAttributes.tuned_transmission) {
                price += (db[`transmission_tuner`]?.price ?? 0) * ExtraAttributes.tuned_transmission * 0.5;
                calculation.push({ type: 'Tuned Transmission', value: (db[`transmission_tuner`]?.price ?? 0) * ExtraAttributes.tuned_transmission * 0.5, count: ExtraAttributes.tuned_transmission });
            }

            //ABILITY SCROLLS
            if (ExtraAttributes.ability_scroll) {
                ExtraAttributes.ability_scroll.forEach((scroll) => {
                    price += (db[scroll.toLowerCase()]?.price ?? 0) * 0.5;
                    calculation.push({ type: 'Scroll', id: scroll, value: (db[scroll.toLowerCase()]?.price ?? 0) * 0.5 });
                });
            }
            ...
            if (ExtraAttributes.hot_potato_count) {
                if (ExtraAttributes.hot_potato_count > 10) {
                    ...
                    calculation.push({ type: 'Fuming Potato Books', value: (db['fuming_potato_book']?.price || 0) * (ExtraAttributes.hot_potato_count - 10), count: ExtraAttributes.hot_potato_count - 10 });
                } else {
            ...
            if (ExtraAttributes.art_of_war_count) {
                price += (db['book_of_stats']?.price || 0) * ExtraAttributes.art_of_war_count * 0.6;
                calculation.push({ type: 'The Art of War', value: db['the_art_of_war']?.price || 0 * ExtraAttributes.art_of_war_count, count: ExtraAttributes.art_of_war_count });
            }

            //BOOK OF STATS
            if (ExtraAttributes.stats_book) {
                price += (db['book_of_stats']?.price || 0) * ExtraAttributes.book_of_stats * 0.6;
                calculation.push({ type: 'Book of Stats', value: db['book_of_stats']?.price || 0 * ExtraAttributes.book_of_stats, count: ExtraAttributes.book_of_stats });
            }
            ...
            if (ExtraAttributes.farming_for_dummies_count) {
                ...
                calculation.push({ type: 'Farming for Dummies', value: (db['farming_for_dummies']?.price || 0) * ExtraAttributes.farming_for_dummies_count, count: ExtraAttributes.farming_for_dummies_count });
            }
            ...
            if (itemId === 'divan_chestplate' || itemId === 'divan_leggings' || itemId === 'divan_boots' || itemId === 'divan_helmet') {
                if (ExtraAttributes?.gems?.unlocked_slots) {
                    ...
                    calculation.push({ type: 'Gemstone Chamber', value: db['gemstone_chamber']?.price * ExtraAttributes.gems.unlocked_slots.length, count: ExtraAttributes.gems.unlocked_slots.length });
                }
            }
