...
const getPricesFromDb = function (pet, db) {
  let lvl1 = db[`lvl_1_${pet.tier}_${pet.type}`.toLowerCase()];
  let lvl100 = db[`lvl_100_${pet.tier}_${pet.type}`.toLowerCase()];
  let lvl200 = db[`lvl_200_${pet.tier}_${pet.type}`.toLowerCase()];

  if (pet.skin) {
    lvl1 = db[`lvl_1_${pet.tier}_${pet.type}_skinned_${pet.skin}`.toLowerCase()] || lvl1;
    lvl100 = db[`lvl_100_${pet.tier}_${pet.type}_skinned_${pet.skin}`.toLowerCase()] || lvl100;
    lvl200 = db[`lvl_200_${pet.tier}_${pet.type}_skinned_${pet.skin}`.toLowerCase()] || lvl200;
  }
