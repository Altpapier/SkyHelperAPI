    ...
    Object.keys(auctions).forEach(async (item) => {
        ...
        if (sales.length && !ignored.includes(item.toUpperCase())) {
            ...
            if (item.includes('skinned') && sales.length > 2) {
                ...
                if (!item.includes('candy') && !(auction?.name || '').toLowerCase().includes('null')) await db.auctions.updateOne({ id: item.toUpperCase() }, { sales: sales, auction: auction }, { upsert: true });
            }
        }
    });
...
const getAttributes = function (item, itemName) {
    ...
    if (itemId == 'PET') {
        ...
        if (data.level == 1 || data.level == 100 || data.level == 200) {
            ...
            if (pet.candyUsed > 0) itemId = `${itemId}_candy_${pet.candyUsed}`;
